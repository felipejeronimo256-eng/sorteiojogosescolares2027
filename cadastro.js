...
// ------------------------------------------------------------------
// Verifica se a escola já está inscrita (bloqueio definitivo)
// ------------------------------------------------------------------
...
async function verificarEscolaJaInscrita(nomeEscola) {
  const idEscola = normalizarNomeEscola(nomeEscola);
  const referencia = db.collection('escolas_inscritas').doc(idEscola);
  const documento = await referencia.get();
  return { existe: documento.exists, referencia, idEscola };
}
...
form.addEventListener('submit', async (e) => {
  ...
  try {
    const { existe, referencia } = await verificarEscolaJaInscrita(escola);

    if (existe) {
      statusMsg.textContent =
        `A escola "${escola}" já está inscrita. Cada escola pode se inscrever apenas uma vez.`;
      statusMsg.className = 'status mostrar falha';
      mostrarErro('escola', 'erro-escola');
      btnEnviar.disabled = false;
      btnEnviar.textContent = 'Enviar inscrição';
      return;
    }

    const inscricaoRef = await db.collection(COLLECTIONS.inscricoes).add({
      nome,
      cpf,
      escola,
    ...
    // trava a escola definitivamente, usando o nome como ID fixo
